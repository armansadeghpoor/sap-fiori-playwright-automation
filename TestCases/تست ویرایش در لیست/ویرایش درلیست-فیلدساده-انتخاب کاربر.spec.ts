...
test("test", async ({ page }) => {
  ...
  await page.locator('a').filter({ hasText: '‫کاربر2‬' }).click();
  await page
    .getByRole("cell", { name: "کاربر2 Select Options" })
    .getByLabel("Select Options")
    .click();
  await page.getByText("کاربر1").click();
  await page.getByTitle('ویرایش در لیست').click();
  await page.getByRole("button", { name: "" }).click();
  await expect(page.locator("bsu-barsa-table-row")).toContainText("‫کاربر1‬");
});
