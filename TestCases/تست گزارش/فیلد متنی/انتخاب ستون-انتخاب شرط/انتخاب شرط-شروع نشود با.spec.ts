...
test("test", async ({ page }) => {
  ...
  await page.getByText("فیلد متنی- انتخاب شرط-شروع نشود با").click();
  await page.getByRole("button", { name: "جدید" }).click();
  await page.getByRole("textbox").click();
  await page.getByRole("textbox").fill("آوا");
  ...
  await page.getByRole("button", { name: "ذخیره و بستن" }).click();
  await expect(page.locator('bsu-ui-table-view')).toContainText('نام سن ‫تست انتخاب شرط کد‬‫‪18‬‫تست‬‫‬');
});
