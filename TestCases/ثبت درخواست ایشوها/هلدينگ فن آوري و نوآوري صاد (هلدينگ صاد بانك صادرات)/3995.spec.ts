...
test('test', async ({ page }) => {
    ...
    await page.locator('input[type="text"]').last().fill('-23');
    await page.locator('input[type="text"]').last().press('Tab');
    await page.locator('bsu-ui-num-decimal-ui').getByRole('textbox').fill('-23');
    await expect(page.locator('input[type="text"]')).toHaveValue('-23');
    await expect(page.locator('bsu-ui-num-decimal-ui').getByRole('textbox')).toHaveValue('-23');
    await page.locator('input[type="text"]').click();
    await page.locator('input[type="text"]').fill('-595');
    await page.locator('input[type="text"]').press('Tab');
    await page.locator('bsu-ui-num-decimal-ui').getByRole('textbox').fill('-595');
    await expect(page.locator('input[type="text"]')).toHaveValue('-595');
