...
test("test", async ({ page }) => {
  ...
  await page.locator("bsu-ui-text-field").getByRole("textbox").fill("10");
  ...
  await page.getByRole("textbox").fill("آزیتا");
  await page.getByRole("button", { name: "ذخیره و بستن" }).click();
  await page.getByRole("button", { name: "" }).nth(1).click();
  await page.getByTitle("آزیتا").locator("div").click();
  await page.getByRole("button", { name: "تایید" }).click();
  await page.getByRole("button", { name: "ذخیره و بستن" }).click();
  await expect(page.getByText('‫آزیتا‬')).toBeVisible();
});
